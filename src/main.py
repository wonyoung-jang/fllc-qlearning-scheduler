...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        # Initializations
        self.minimum_slots_required = update_minimum_slots_required(
            self.num_teams,
            self.num_rooms,
            self.num_tables_and_sides,
            self.round_types_per_team,
        )
        (
            self.available_practice_duration,
            self.available_table_duration,
            self.round_type_durations,
        ) = update_round_type_durations(
        ...
        self.round_type_time_slots = create_round_type_time_slots(
            self.judging_round_start_times,
            self.practice_round_start_times,
            self.table_round_start_times,
            self.practice_rounds_start_time,
    ...
    def create_schedule_data_inputs(self):
        ...
        self.num_tables_and_sides_label = QLabel(str(self.num_tables_and_sides))
        ...
        for name, spinbox in self.round_type_spinboxes.items():
            self.round_type_labels[name] = QLabel(
                f"{self.num_teams * self.round_types_per_team[name]} Rounds"
            )
    ...
    def initialize_schedule_data_inputs(self):
        ...
        """
        self.num_teams_spinbox.setValue(self.num_teams)
        self.num_rooms_spinbox.setValue(self.num_rooms)
        self.num_tables_spinbox.setValue(self.num_tables)
        for name, spinbox in self.round_type_spinboxes.items():
            if name == KEY.JUDGING:
                continue
            spinbox.setValue(self.round_types_per_team[name])
    ...
    def setup_schedule_data_inputs(self):
        ...
        self.schedule_data_layout.addWidget(
            QLabel("Number of Tables and Sides: "), 3, 0
        )
        self.schedule_data_layout.addWidget(
            self.num_tables_and_sides_label, 3, 1, Qt.AlignRight
        )
        ...
        for name, spinbox in self.round_type_spinboxes.items():
            ...
            self.round_type_layout.addWidget(
                self.round_type_labels[name], count, 2, Qt.AlignRight
            )
            count += 1
    ...
    def create_time_data_inputs(self):
        ...
        self.practice_round_duration = QLabel(
            f"{self.round_type_durations[KEY.PRACTICE]} minutes"
        )
        self.table_round_duration = QLabel(
            f"{self.round_type_durations[KEY.TABLE]} minutes"
        )
        ...
        self.practice_time_available = QLabel(
            f"{self.available_practice_duration} minutes"
        )
        self.table_time_available = QLabel(f"{self.available_table_duration} minutes")
    ...
    def initialize_time_data_inputs(self):
        ...
        """
        self.minimum_practice_duration.setTime(
            QTime.fromString(str(self.round_type_durations[KEY.PRACTICE]), "mm")
        )
        self.minimum_table_duration.setTime(
            QTime.fromString(str(self.round_type_durations[KEY.TABLE]), "mm")
        )
        self.start_time_judging_rounds.setTime(
            QTime.fromString(self.judging_rounds_start_time, "HH:mm")
        )
        self.start_time_practice_rounds.setTime(
            QTime.fromString(self.practice_rounds_start_time, "HH:mm")
        )
        self.stop_time_practice_rounds.setTime(
            QTime.fromString(self.practice_rounds_stop_time, "HH:mm")
        )
        self.start_time_table_rounds.setTime(
            QTime.fromString(self.table_rounds_stop_time, "HH:mm")
        )
        self.stop_time_table_rounds.setTime(
            QTime.fromString(self.table_rounds_stop_time, "HH:mm")
        )
    ...
    def setup_time_data_inputs(self):
        ...
        self.time_data_layout.addWidget(
            self.judging_round_duration_label_constant, 0, 0
        )
        ...
        self.judging_stop_time = self.start_time_judging_rounds.time().addSecs(
    ...
    def create_statistics_and_progress(self):
        ...
        self.q_learning_label = QLabel(
            f"Epsilon: {self.q_learning.epsilon:.2f}\nAlpha: {self.q_learning.learning_rate:.2f}\nGamma: {self.q_learning.discount_factor:.2f}\nEpisodes: {self.q_learning.training_episodes}"
        )
        self.q_table_size_label = QLabel(
            f"Q-Table Size: {len(self.q_learning.q_table)}/{self.q_learning.q_table_size_limit}"
        )
    ...
    def initialize_statistics_and_progress(self):
        ...
        self.current_schedule_length_label.setText(
            f"Required Schedule Slots: {self.q_learning.required_schedule_slots} ({self.q_learning.possible_schedule_slots} Possible)"
        )
    ...
    def create_soft_constraint_weights(self):
        ...
        self.soft_constraint_weights_layout = QGridLayout(
            self.soft_constraint_weights_groupbox
        )
        ...
        for constraint in [
            KEY.TABLE_CONSISTENCY,
            KEY.OPPONENT_VARIETY,
            KEY.BACK_TO_BACK_PENALTY,
            KEY.BREAK_TIME,
        ]:
            # Create the slider for the constraint
            self.soft_constraint_weights[constraint] = QSlider(Qt.Horizontal, self)
            self.soft_constraint_weights[constraint].setRange(
                0, 100
            )  # Adjust this based on the range you need
            self.soft_constraint_weights[constraint].setSingleStep(1)
            self.soft_constraint_weights[constraint].setTickInterval(10)
            self.soft_constraint_weights[constraint].setSliderPosition(
                self.q_learning.soft_constraints_weight[constraint] * scale_factor
            )

            # Connect the valueChanged signal of the slider to the on_update method
            self.soft_constraint_weights[constraint].valueChanged.connect(
                self.on_update
            )

            # Create the label for the constraint
            self.constraint_label[constraint] = QLabel(
                f"{constraint}: {self.q_learning.soft_constraints_weight[constraint] * scale_factor:.2f}%"
            )

            # Add the label and slider to the layout
            self.soft_constraint_weights_layout.addWidget(
                self.constraint_label[constraint], row, 0, Qt.AlignLeft
            )
            self.soft_constraint_weights_layout.addWidget(
                self.soft_constraint_weights[constraint], row, 1, Qt.AlignRight
            )
    ...
    def create_schedule_display(self):
        ...
        self.judging_table.setColumnCount(self.num_rooms + 1)  # Time + Room columns
        self.judging_table.setHorizontalHeaderLabels(
            ["Time"] + [f"Room {i+1}" for i in range(self.num_rooms)]
        )

        # Practice Rounds Table
        self.practice_table = QTableWidget()
        self.practice_table.setColumnCount(
            self.num_tables * 2 + 1
        )  # Time + Table columns
        practice_headers = ["Time"] + [
            f"Table {chr(65 + i // 2)}{i % 2 + 1}" for i in range(self.num_tables * 2)
        ]
        self.practice_table.setHorizontalHeaderLabels(practice_headers)

        # Table Rounds Table
        self.table_round_table = QTableWidget()
        self.table_round_table.setColumnCount(
            self.num_tables * 2 + 1
        )  # Time + Table columns
        table_round_headers = ["Time"] + [
            f"Table {chr(65 + i // 2)}{i % 2 + 1}" for i in range(self.num_tables * 2)
        ]
        ...
        for entry in sorted(self.q_learning.schedule, key=lambda x: (x[0], x[2], x[4])):
            ...
            if time_start not in last_row[round_type]:
                ...
                table_widget.setItem(
                    newRow, 0, QTableWidgetItem(time_start)
    ...
    def clear_and_setup_tables(self):
        ...
        for table in [self.judging_table, self.practice_table, self.table_round_table]:
            ...
            if table is self.judging_table:
                ...
                table.setColumnCount(self.num_rooms + 1)
                table.setHorizontalHeaderLabels(
                    ["Time"] + [f"Room {i+1}" for i in range(self.num_rooms)]
                )
            else:
                # Set column count and header labels for practice and table round tables
                table.setColumnCount((self.num_tables * 2) + 1)
                table.setHorizontalHeaderLabels(
                    ["Time"]
                    + [
                        f"Table {chr(65 + i // 2)}{i % 2 + 1}"
                        for i in range(self.num_tables * 2)
                    ]
                )
    ...
    def create_submission_buttons(self):
        ...
        self.train_button = QPushButton(
            "Train then Generate Optimal Schedule (Ctrl + G)", self
        )
    ...
    def connect_signals_and_slots(self):
        ...
        """
        # Time Data Inputs
        self.start_time_judging_rounds.timeChanged.connect(
            self.on_update
        )  # Connects the timeChanged signal of start_time_judging_rounds to the on_update slot function

        self.start_time_practice_rounds.timeChanged.connect(
            self.on_update
        )  # Connects the timeChanged signal of start_time_practice_rounds to the on_update slot function
        self.stop_time_practice_rounds.timeChanged.connect(
            self.on_update
        )  # Connects the timeChanged signal of stop_time_practice_rounds to the on_update slot function

        self.minimum_practice_duration.timeChanged.connect(
            self.on_update
        )  # Connects the timeChanged signal of minimum_practice_duration to the on_update slot function
        self.minimum_table_duration.timeChanged.connect(
            self.on_update
        )  # Connects the timeChanged signal of minimum_table_duration to the on_update slot function

        self.start_time_table_rounds.timeChanged.connect(
            self.on_update
        )  # Connects the timeChanged signal of start_time_table_rounds to the on_update slot function
        self.stop_time_table_rounds.timeChanged.connect(
            self.on_update
        )  # Connects the timeChanged signal of stop_time_table_rounds to the on_update slot function

        self.start_time_practice_rounds.timeChanged.connect(
            self.validate_practice_times
        )  # Connects the timeChanged signal of start_time_practice_rounds to the validate_practice_times slot function
        self.stop_time_practice_rounds.timeChanged.connect(
            self.validate_practice_times
        )  # Connects the timeChanged signal of stop_time_practice_rounds to the validate_practice_times slot function
        self.minimum_practice_duration.timeChanged.connect(
            self.validate_practice_times
        )  # Connects the timeChanged signal of minimum_practice_duration to the validate_practice_times slot function

        self.start_time_table_rounds.timeChanged.connect(
            self.validate_table_times
        )  # Connects the timeChanged signal of start_time_table_rounds to the validate_table_times slot function
        self.stop_time_table_rounds.timeChanged.connect(
            self.validate_table_times
        )  # Connects the timeChanged signal of stop_time_table_rounds to the validate_table_times slot function
        self.minimum_table_duration.timeChanged.connect(
            self.validate_table_times
        )  # Connects the timeChanged signal of minimum_table_duration to the validate_table_times slot function

        # Schedule Data Inputs
        self.num_teams_spinbox.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of num_teams_spinbox to the on_update slot function
        self.num_rooms_spinbox.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of num_rooms_spinbox to the on_update slot function
        self.num_tables_spinbox.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of num_tables_spinbox to the on_update slot function
        self.round_type_spinboxes[KEY.PRACTICE].valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of round_type_spinboxes[KEY.PRACTICE] to the on_update slot function
        self.round_type_spinboxes[KEY.TABLE].valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of round_type_spinboxes[KEY.TABLE] to the on_update slot function

        # Q-Learning Inputs
        self.alpha_input.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of alpha_input to the on_update slot function
        self.gamma_input.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of gamma_input to the on_update slot function
        self.epsilon_start_input.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of epsilon_start_input to the on_update slot function
        self.epsilon_end_input.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of epsilon_end_input to the on_update slot function
        self.epsilon_decay_input.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of epsilon_decay_input to the on_update slot function
        self.training_episodes_input.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of training_episodes_input to the on_update slot function

        # Statistics and Progress
        self.gui_refresh_rate.valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of gui_refresh_rate to the on_update slot function
        self.soft_constraint_weights[KEY.TABLE_CONSISTENCY].valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of soft_constraint_weights[KEY.TABLE_CONSISTENCY] to the on_update slot function
        self.soft_constraint_weights[KEY.OPPONENT_VARIETY].valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of soft_constraint_weights[KEY.OPPONENT_VARIETY] to the on_update slot function
        self.soft_constraint_weights[KEY.BACK_TO_BACK_PENALTY].valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of soft_constraint_weights[KEY.BACK_TO_BACK_PENALTY] to the on_update slot function
        self.soft_constraint_weights[KEY.BREAK_TIME].valueChanged.connect(
            self.on_update
        )  # Connects the valueChanged signal of soft_constraint_weights[KEY.BREAK_TIME] to the on_update slot function

        # Buttons
        self.train_button.clicked.connect(
            self.start_training_thread
        )  # Connects the clicked signal of train_button to the start_training_thread slot function
    ...
    def start_training_thread(self):
        ...
        training_episodes = int(
            self.training_episodes_input.value()
        )  # Assuming you have an input field for training_episodes
        ...
        self.worker.signals.update_gui_signal.connect(
            self.update_gui_total, Qt.QueuedConnection
        )
                ...
                for col in range(table_widget.columnCount()):
                    ...
                    if curr_item is not None and curr_item.text() == value:
                        ...
                        if curr_item.text() != "None" and ":" not in curr_item.text():
                            curr_item.setBackground(
                                QBrush(self.color_map.get(int(curr_item.text())))
                            )
    ...
    def validate_practice_times(self):
        ...
        duration = self.round_type_durations[KEY.PRACTICE]
        ...
        if duration < min_duration:
            ...
            end_practice = end_practice.addSecs(
                (min_duration.minute() - duration.minute()) * TIME.SECS_IN_MINUTE
            )
    ...
    def validate_table_times(self):
        ...
        duration = self.round_type_durations[KEY.TABLE]
        duration = QTime(0, duration, 0)

        if duration < min_duration:
            end_table = end_table.addSecs(
                (min_duration.minute() - duration.minute()) * TIME.SECS_IN_MINUTE
            )
    ...
    def update_schedule_data(self):
        ...
        """
        self.num_teams = self.num_teams_spinbox.value()
        self.num_rooms = self.num_rooms_spinbox.value()
        self.num_tables = self.num_tables_spinbox.value()
        self.round_types_per_team[KEY.PRACTICE] = self.round_type_spinboxes[
            KEY.PRACTICE
        ].value()
        self.round_types_per_team[KEY.TABLE] = self.round_type_spinboxes[
            KEY.TABLE
        ].value()
        self.num_tables_and_sides = self.num_tables * 2
        self.num_tables_and_sides_label.setText(str(self.num_tables_and_sides))
        for name in [KEY.JUDGING, KEY.PRACTICE, KEY.TABLE]:
            self.round_type_labels[name].setText(
                f"{self.num_teams * self.round_types_per_team[name]} Rounds"
            )
    ...
    def update_time_data(self):
        ...
        """
        jStop = self.start_time_judging_rounds.time().addSecs(
            self.round_type_durations[KEY.JUDGING]
            * TIME.SECS_IN_MINUTE
            * self.minimum_slots_required[KEY.JUDGING]
        )
        ...
        self.judging_rounds_start_time = self.start_time_judging_rounds.time().toString(
            "hh:mm"
        )
        self.practice_rounds_start_time = (
            self.start_time_practice_rounds.time().toString("hh:mm")
        )
        self.practice_rounds_stop_time = self.stop_time_practice_rounds.time().toString(
            "hh:mm"
        )
        self.table_rounds_stop_time = self.start_time_table_rounds.time().toString(
            "hh:mm"
        )
        self.table_rounds_stop_time = self.stop_time_table_rounds.time().toString(
            "hh:mm"
        )
    ...
    def on_update(self):
        ...
        for constraint in self.soft_constraint_weights:
            self.q_learning.soft_constraints_weight[constraint] = (
                self.soft_constraint_weights[constraint].value() / 100.0
            )
            self.constraint_label[constraint].setText(
                f"{constraint}: {self.q_learning.soft_constraints_weight[constraint] * 100:.2f}%"
            )
        ...
        self.progress_bar.setMaximum(self.q_learning.training_episodes)
        self.q_learning.required_schedule_slots = (
            sum(self.round_types_per_team.values()) * self.num_teams
        )
        self.q_learning.possible_schedule_slots = (
            self.num_rooms * self.minimum_slots_required[KEY.JUDGING]
            + self.num_tables_and_sides
            * (
                self.minimum_slots_required[KEY.PRACTICE]
                + self.minimum_slots_required[KEY.TABLE]
        ...
        self.q_learning.q_table_size_limit = (
            len(self.q_learning.states) * self.num_teams
        )
        ...
        self.q_table_size_label.setText(
            f"Q-Table Size: {len(self.q_learning.q_table)}/{self.q_learning.q_table_size_limit}"
        )

        self.q_learning.practice_teams_available = (
            list(self.teams.keys()) * self.round_types_per_team[KEY.PRACTICE]
        )
        self.q_learning.table_teams_available = (
            list(self.teams.keys()) * self.round_types_per_team[KEY.TABLE]
        )

        # Update TimeData with current GUI inputs
        self.practice_round_duration.setText(
            f"{self.round_type_durations[KEY.PRACTICE]} minutes"
        )
        self.table_round_duration.setText(
            f"{self.round_type_durations[KEY.TABLE]} minutes"
        )
        self.practice_time_available.setText(
            f"{self.available_practice_duration} minutes"
        )
        self.table_time_available.setText(f"{self.available_table_duration} minutes")
    ...
    def update_gui_total(self, episode):
        ...
        if episode == -2:  # Optimal
            ...
            self.q_learning_label.setText(
                f"Epsilon: {self.q_learning.epsilon:.2f} (Final)\nAlpha: {self.q_learning.learning_rate:.2f}\nGamma: {self.q_learning.discount_factor:.2f}\nEpisodes: {self.q_learning.training_episodes}"
            )
            self.q_table_size_label.setText(
                f"Q-Table Size: {len(self.q_learning.q_table)}/{self.q_learning.q_table_size_limit} (Final)"
            )
            ...
            self.train_button.setText(
                f"Training in Progress...{episode}/{self.q_learning.training_episodes}"
            )
            self.worker.signals.gui_updated_signal.emit()
