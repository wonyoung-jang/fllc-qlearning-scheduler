...
class QLearningExporter:
    ...
    def __init__(self):
        ...
        self.dataframe = None
        self.excel_file_path = EXPORT.EXPORTS_DIRECTORY + EXPORT.OPTIMAL_SCHEDULE_EXCEL_FILENAME + EXPORT.XLSX_EXT
    ...
    def transform_dataframe_to_grid(self, dataframe, sheet_name, writer):
        ...
        """
        pivot_dataframe = dataframe.pivot_table(index=EXPORT.COL_TIME, columns=[EXPORT.COL_LOCATION], values=EXPORT.COL_TEAM).reset_index()
    ...
    def export_q_table_to_csv(self, file_path, q_table):
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as file:
            ...
            writer.writerow([EXPORT.COL_TIME, EXPORT.COL_ROUND, EXPORT.COL_LOCATION, EXPORT.COL_TEAM, EXPORT.COL_QVALUE])
    ...
    def export_schedule_to_csv(self, file_path, schedule):
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerow([EXPORT.COL_TIME, EXPORT.COL_ROUND, EXPORT.COL_LOCATION, EXPORT.COL_TEAM])
    ...
    def export_optimal_schedule_to_excel(self, file_path, schedule):
        ...
        with pd.ExcelWriter(self.excel_file_path, engine='xlsxwriter') as writer:
            for round_type in self.dataframe[EXPORT.COL_ROUND].unique():
                filtered_dataframe = self.dataframe[self.dataframe[EXPORT.COL_ROUND] == round_type]
                self.transform_dataframe_to_grid(filtered_dataframe, round_type, writer)
                workbook = writer.book
                worksheet = writer.sheets[round_type]
                dataframe_for_sheet = self.dataframe[self.dataframe[EXPORT.COL_ROUND] == round_type].dropna()
