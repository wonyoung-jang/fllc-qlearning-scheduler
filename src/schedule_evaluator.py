...
class ScheduleEvaluator:
    ...
    def calculate_round_length_by_start_times(self, data_summary):
        ...
        round_average_lengths = {round_type: sum(durations) / len(durations) for round_type, durations in round_lengths.items() if durations}

        for round_type, start_times in round_start_times.items():
            for start_time in start_times:
                round_end_times[round_type].append(datetime.strptime(start_time, "%H:%M") + timedelta(minutes=round_average_lengths[round_type]))
    ...
    def extract_detailed_information(self):
        ...
        detailed_tracking['Round Average Length'], detailed_tracking['Round Start Times'], detailed_tracking['Round End Times'] = self.calculate_round_length_by_start_times(detailed_tracking)
    ...
    def calculate_team_data(self, data_summary, team):
        ...
        trl = [(data_summary['Time']['All'][i], data_summary['Time']['End'][i], data_summary['Round']['All'][i], data_summary['Location']['All'][i]) for i in appearances]
        ...
        if len(times) > 1:
            ...
            break_between_end_times = [(end_times[i+1] - end_times[i]).total_seconds() / 60 for i in range(len(times) - 1)]
            ...
            team_data['Average Break Between Start Times'] = sum(break_between_start_times) / len(break_between_start_times)
            ...
            team_data['Average Break Time'] = (team_data['Average Break Between Start Times'] + team_data['Average Break Between End Times']) + team_data['Average Downtime']/ 3
        ...
        for appearance in team_data['Appearances']:
            ...
            if 0 <= opponent_index < len(data_summary['Team']['All']) and data_summary['Team']['All'][opponent_index] != 0:  # Check for 'None' 
    ...
    def calculate_schedule_score(self, completion_percentage):
        """Calculate a comprehensive score for the schedule based on various metrics,
        factoring in the percentage of the schedule that has been completed.

        :param completion_percentage: A float between 0 and 1 representing the percentage of the schedule completed.
        """
        ...
        avg_downtime, max_downtime, avg_appearances, min_appearances, max_appearances, appearance_variance, avg_unique_locations, avg_unique_opponents = self.calculate_base_metrics(team_stats)
        ...
        normalized_location_diversity = self.normalize(avg_unique_locations, 0, len(self.detailed_data_summary['Location']['Unique']))
        normalized_opponent_diversity = self.normalize(avg_unique_opponents, 0, len(self.detailed_data_summary['Team']['Unique']))
    ...
    def calculate_base_metrics(self, team_stats):
        ...
        avg_downtime = sum(info['Average Downtime'] for info in team_stats.values() if info['Average Downtime'] != []) / len(team_stats)
        ...
        appearance_variance = sum((info['Count Appearances'] - avg_appearances) ** 2 for info in team_stats.values()) / len(team_stats)
        ...
        return avg_downtime, max_downtime, avg_appearances, min_appearances, max_appearances, appearance_variance, avg_unique_locations, avg_unique_opponents
