...
class QLearning():    
    def __init__(self, schedule_data, time_data):        
        ...
        # Schedule and time data references
        self.schedule_data = schedule_data
        self.time_data = time_data
        self.required_schedule_slots = self.schedule_data.num_teams * sum(self.schedule_data.round_types_per_team.values())
        self.possible_schedule_slots = self.schedule_data.num_rooms * self.time_data.min_slots_required["judging"] + self.schedule_data.num_tables_and_sides * (self.time_data.min_slots_required["practice"] + self.time_data.min_slots_required["table"])
        ...
        self.max_num_rounds = sum(self.schedule_data.round_types_per_team.values())
            
        self.exporter = QLearningExporter()

    # Initialize the schedule and states
    def initialize_schedule_and_states(self):
        ...
        self.practice_teams_available = list(self.schedule_data.teams.keys()) * self.schedule_data.round_types_per_team["practice"]
        self.table_teams_available = list(self.schedule_data.teams.keys()) * self.schedule_data.round_types_per_team["table"]        
        self.current_schedule_length = 0
        
    def initialize_schedule(self):
        self.schedule_data.teams, self.schedule_data.rooms, self.schedule_data.tables = self.schedule_data.initialize_teams_rooms_tables()
        schedule = []
        for round_type, slots in self.time_data.time_slots.items():
            for time_start, time_end in slots:
                ...
                else:  # practice and table rounds
                    for table, details in self.schedule_data.tables.items():
                        table_id, side = table
                        schedule.append([time_start, time_end, round_type, "table", f"{table_id}{side}", None])
        return schedule

    def initialize_judging_rounds(self):
        # Track the current team being assigned
        current_team_id = list(self.schedule_data.teams.keys())[0]

        for i, schedule in enumerate(self.schedule):
            # Only fill teams for judging rounds
            if schedule[2] == "judging" and current_team_id <= len(list(self.schedule_data.teams.keys())):
                self.schedule[i][5] = current_team_id  # Assign the current team ID to the team_id column
                self.schedule_data.teams[current_team_id]["scheduled_round_types"]["judging"] += 1  # Increment the judging round count for the current team
                self.schedule_data.teams[current_team_id]["scheduled_times"].append((schedule[0], schedule[1]))  # Add the start time to the scheduled times
                self.schedule_data.rooms[schedule[4]]["scheduled_teams"].append(current_team_id)  # Add the current team to the scheduled teams for the room
                current_team_id += 1  # Move to the next team

        return self.schedule

    # Train benchmark episodes for a baseline:
    def train_benchmark_episodes(self):
        self.initialize_schedule_and_states()
        self.current_schedule_length = 0
        ...
        while self.states:     
            ...
            if actions:
                selected_action = random.choice(actions)  # Randomly select an action
                if selected_action is not None:
                    self.update_team_availability(selected_action, current_round_type, (current_start_time, current_end_time), current_location_letter, current_location_side)
                    self.update_table_availability(current_location_id, selected_action, (current_start_time, current_end_time))
                    self.update_schedule(current_state, selected_action)
                    self.states.remove(current_state)
                    self.staticStates[self.staticStates.index(current_state)] = (current_start_time, current_end_time, current_round_type, current_location_type, f'{current_location_letter}{current_location_side}', selected_action)
        ...
        self.exporter.export_schedule_for_eval_to_csv(scheduleCSVFileName, self.schedule)
    ...
    # Train one episode - Used by the training worker        
    def train_one_episode(self, episode):
        ...
        while self.states:
            ...
            actions = self.update_available_actions(current_state)
            if actions:
                selected_action = self.select_action(current_state, actions) # Select an action using the epsilon-greedy policy
                if selected_action is not None:
                    self.update_team_availability(selected_action, current_round_type, (current_start_time, current_end_time), current_location_letter, current_location_side)
                    self.update_table_availability(current_location_id, selected_action, (current_start_time, current_end_time))
                    self.update_schedule(current_state, selected_action)
                    ...
                    self.states.remove(current_state)
                    self.staticStates[self.staticStates.index(current_state)] = (current_start_time, current_end_time, current_round_type, current_location_type, current_location_id, selected_action)
                    ...
                    if next_state is not None:
                        ...
                        self.q_table[(current_state, selected_action)] = (1 - self.alpha) * self.q_table.get((current_state, selected_action), 0) + self.alpha * reward
                else:
    ...
    def generate_optimal_schedule(self): 
        # Reset the environment to its initial state for scheduling
        self.initialize_schedule_and_states()
        self.current_schedule_length = 0
        ...
        while self.states:
            ...
            if available_actions:
                best_action = max(available_actions, key=lambda action: self.q_table.get((current_state, action), float("-inf")))
                if best_action:
                    self.update_team_availability(best_action, current_state[2], (current_state[0], current_state[1]), current_state[4][0], current_state[4][1])
                    self.update_table_availability(current_state[4], best_action, (current_state[0], current_state[1]))
                    self.update_schedule(current_state, best_action)
                    
                    self.staticStates[self.staticStates.index(current_state)] = (current_state[0], current_state[1], current_state[2], current_state[3], current_state[4], best_action)
        ...
        self.exporter.export_optimal_schedule_to_csv(optimal_filename, self.schedule)
    ...
    def update_available_actions(self, state): # Hard Constraints
        """
        Hard Constraints
        It asks: "What teams are available for the current state?"
        To answer it considers each possible team (all teams in tournament)
        For each it asks:
            1. "Is the team already scheduled for the current round type for rounds_per_type number of times?"
                If yes: remove the team from the list of available actions
            ...
            3. "Is the current state's table side == 2?"
                If yes: ask:
                    3a. "Is the table side 1 of the previous state scheduled?"
                        If no: 
                            no available_actions
                        If yes: ask:
                            3b. "Is there 1 or more available actions?"
                                If no: 
                                    no available_actions
                                    update team, table and schedule of previous state
        """
        ...
        for team_id in potential_actions:
            if self.schedule_data.teams[team_id]["scheduled_round_types"][round_type] >= self.schedule_data.round_types_per_team[round_type]:
    ...
    def update_team_availability(self, team_id, round_type, time_slot, location_id, side):
        if team_id in self.schedule_data.teams:
            ...
            if side == '2':  # Assuming side 2 indicates both are scheduled
                ...
                for other_team_id, other_team_info in self.schedule_data.teams.items():
                    if other_team_id != team_id and (time_slot, (table_id, other_side)) in other_team_info["scheduled_time_table_pairs"]:
    ...
    def update_table_availability(self, location_id, team_id, time_slot):
    ...
    def update_schedule(self, current_state, selected_action):
    ...
    def update_q_value(self, state, action, reward, next_state, actions):
        # Initialize Q-value if the state-action pair is not in the q_table-table
        current_q = self.q_table.get((state, action), 10)

        # Q-Learning formula
        max_future_q = max([self.q_table.get((next_state, a), 10) for a in actions], default=10)
        new_q = (1 - self.alpha) * current_q + self.alpha * (reward + self.gamma * max_future_q)
        self.q_table[(state, action)] = new_q

    # Update available actions helper functions                             
    def find_previous_state(self, state):
    ...
    def update_previous_state(self, prev_state):
        prev_time_start, prev_time_end, prev_round_type, prev_location_type, prev_location_id, prev_team_id = prev_state
    ...
    def get_reward(self, state, action):  # Soft Constraints
        """
        Soft Constraints
        It asks: "How good is the current state-action pair?"
        To answer it considers each possible team (all teams in tournament)
        For each it asks:
            1. "Does the team have the same table for all 2 practice and all 3 table rounds?"
            2. "Does the team have a different opponent for all 2 practice and all 3 table rounds?"
            3. "Is the team scheduled for rounds back to back?"
            4. "Does the team have a break between each of their judging, practice, and table rounds?"
        """        
        reward = 0  # Default reward for each action
        time_start, time_end, round_type, location_type, location_id, team_id = state
        team_id, scheduled_round_types, scheduled_times, scheduled_tables, scheduled_opponents, scheduled_time_table_pairs = self.schedule_data.teams[action].values()
        table_key = (location_id[0], int(location_id[1]))
        timeStateStart = self.time_data.time_to_minutes(time_start)
        timeStateEnd = self.time_data.time_to_minutes(time_end)

        # 1a. Does the team have the same table for all 2 practice and all 3 table rounds?
        ...
        if length_of_tables_scheduled > 1:
            ...
            reward_table = unique_to_tables / self.max_num_rounds  # Original reward calculation
            # Normalize this reward
            min_reward_table = 0  # Define based on expected range
            max_reward_table = 1  # Define based on expected range
            reward_table_normalized = self.normalize_reward(reward_table, min_reward_table, max_reward_table)
            reward += reward_table_normalized * self.constraint_weights['Table Consistency']        
        ...
        if length_of_opponents_scheduled > 1:
            ...
            reward_opponent = unique_to_opponents * self.max_num_rounds * self.constraint_weights['Opponent Variety']
            min_reward_opponent = 0  # Define based on expected range
            max_reward_opponent = 1
            reward_opponent_normalized = self.normalize_reward(reward_opponent, min_reward_opponent, max_reward_opponent)
        ...
        for i in range(len(scheduled_times)):
            ...
            if timeActionStart - timeStateEnd <= 0 or timeActionEnd - timeStateStart <= 0:
                reward_back_to_back -= 1
            else:
                reward_back_to_back += 1
                
            if timeStateStart - timeActionEnd <= 0 or timeStateEnd - timeActionStart <= 0:
                reward_back_to_back -= 1
            else:
                reward_back_to_back += 1
            ...
            reward_back_to_back_normalized = self.normalize_reward(reward_back_to_back, min_reward_back_to_back, max_reward_back_to_back)
            reward += reward_back_to_back_normalized * self.constraint_weights['Back to Back Penalty']
        ...
        for i in range(1, len(scheduled_times)):
            ...
            timeActionEnd = self.time_data.time_to_minutes(scheduled_times[i][1])
            reward_break_time = 0
            if timeActionStart - timeStateEnd >= break_time or timeActionEnd - timeStateStart >= break_time:
                reward_break_time += 1 
            else:
                reward_break_time -= 1
                
            if timeStateStart - timeActionEnd >= break_time or timeStateEnd - timeActionStart >= break_time:
                reward_break_time += 1
            else:
                reward_break_time -= 1
            ...
            reward_break_time_normalized = self.normalize_reward(reward_break_time, min_reward_break_time, max_reward_break_time)
            reward += reward_break_time_normalized * self.constraint_weights['Break Time']

        completion_reward = self.current_schedule_length / (self.required_schedule_slots - (self.schedule_data.num_teams * self.schedule_data.round_types_per_team["judging"]))
        return reward + (reward * completion_reward)
            
    # Schedule Display in main.py Slot helper functions
    def transform_for_heatmap(self):
        data = {}
        for ((start_time, end_time, round_type, location_type, location_id, team_in_state), action), value in self.q_table.items():
            # Use a more detailed state representation
            detailed_state = (start_time[:5], round_type)  # Simplify start_time and combine with round_type
    ...
    def aggregated_data_to_2d_array(self, aggregated_data):
        # Extract unique detailed states and actions
